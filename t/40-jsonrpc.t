...
subtest 'submatrix without test param returns -32602' => sub {
    my $res = $t->post_ok('/api',
        json => { jsonrpc => '2.0', id => 20, method => 'submatrix', params => {} })
        ->status_is(200)
        ->json_is('/error/code' => -32602)
        ->tx->res->json;
    like $res->{error}{message}, qr/test/, 'error names the missing param';
};

subtest 'reports_from_id without rid returns -32602' => sub {
    $t->post_ok('/api',
        json => { jsonrpc => '2.0', id => 21, method => 'reports_from_id', params => {} })
        ->status_is(200)
        ->json_is('/error/code' => -32602);
};

subtest 'reports_from_date without epoch returns -32602' => sub {
    $t->post_ok('/api',
        json => { jsonrpc => '2.0', id => 22, method => 'reports_from_date', params => {} })
        ->status_is(200)
        ->json_is('/error/code' => -32602);
};
