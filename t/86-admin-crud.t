...
subtest 'cancel token without CSRF is rejected' => sub {
    my $tokens = $h->app->auth->list_tokens;
    my $id = $tokens->[0]{id};
    ...
    ok $h->app->auth->validate_token($tokens->[0]{token}),
       'token still active after CSRF-less cancel attempt';
};
...
subtest 'cancel token' => sub {
    ...
    $t->post_ok("/admin/tokens/$id/cancel", form => { csrf_token => $csrf })
      ->status_is(302);
